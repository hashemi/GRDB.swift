    ...
    func testRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                let request = reader.makeRequest(Reader.books)
                let books = try request.fetchAll(db)
                XCTAssertEqual(lastSQLQuery, "SELECT \"books\".* FROM \"books\" JOIN \"libraries\" ON ((\"libraries\".\"id\" = \"books\".\"libraryId\") AND (\"libraries\".\"id\" = 1))")
                assertMatch(books, [
            ...
            do {
                ...
                let request = reader.makeRequest(Reader.books)
                let books = try request.fetchAll(db)
                XCTAssertEqual(lastSQLQuery, "SELECT \"books\".* FROM \"books\" JOIN \"libraries\" ON ((\"libraries\".\"id\" = \"books\".\"libraryId\") AND (\"libraries\".\"id\" = 2))")
                assertMatch(books, [
            ...
            do {
                ...
                let request = reader.makeRequest(Reader.books)
                let books = try request.fetchAll(db)
                XCTAssertEqual(lastSQLQuery, "SELECT \"books\".* FROM \"books\" JOIN \"libraries\" ON ((\"libraries\".\"id\" = \"books\".\"libraryId\") AND (\"libraries\".\"id\" = 3))")
                XCTAssertTrue(books.isEmpty)
            }
            
            do {
                let reader = try Reader.fetchOne(db, key: "barbara@example.com")!
                let request = reader.makeRequest(Reader.books).filter(Column("title") != "Walden")
                let books = try request.fetchAll(db)
                XCTAssertEqual(lastSQLQuery, "SELECT \"books\".* FROM \"books\" JOIN \"libraries\" ON ((\"libraries\".\"id\" = \"books\".\"libraryId\") AND (\"libraries\".\"id\" = 1)) WHERE (\"books\".\"title\" <> \'Walden\')")
                assertMatch(books, [
                    ["isbn": "book2", "title": "The Fellowship of the Ring", "libraryId": 1],
                    ["isbn": "book4", "title": "Le Comte de Monte-Cristo", "libraryId": 1],
                    ])
            ...
            do {
                let reader = try Reader.fetchOne(db, key: "barbara@example.com")!
                let request = reader.makeRequest(Reader.books).order(Column("title").desc)
                let books = try request.fetchAll(db)
                XCTAssertEqual(lastSQLQuery, "SELECT \"books\".* FROM \"books\" JOIN \"libraries\" ON ((\"libraries\".\"id\" = \"books\".\"libraryId\") AND (\"libraries\".\"id\" = 1)) ORDER BY \"books\".\"title\" DESC")
                assertMatch(books, [
                    ["isbn": "book3", "title": "Walden", "libraryId": 1],
                    ["isbn": "book2", "title": "The Fellowship of the Ring", "libraryId": 1],
                    ["isbn": "book4", "title": "Le Comte de Monte-Cristo", "libraryId": 1],
